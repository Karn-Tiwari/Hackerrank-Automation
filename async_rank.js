...
(async function(){
    try{
        let browserInstance=await puppeteer.launch({// puppeteer initialize hota hai iss launch function se
    ...
    let newTab = await browserInstance.newPage()//isse login link pe hit karega aur new tab open karke de dega
    await newTab.goto(loginLink)//login link pe hit karega
    ...
    let allChallengesPromise = await newTab.$$('.ui-btn.ui-btn-normal.primary-cta.ui-btn-line-primary.ui-btn-styled',{delay:50})//$=>document.querySelector and $$=>document.querySelectorAll
    console.log('number of questions are',allChallengesPromise.length)// isse pata chelga kya hme wo sare k sare sawal mil gaye
    let questionWillBeSolved = await questionSolver(newTab,allChallengesPromise[0],codeObj.answer[0]);
